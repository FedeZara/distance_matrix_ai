...
exports.inject = function (options) {
  ...
  var key = options.key || process.env.DISTANCE_MATRIX_API_KEY;
  var channel = options.channel;
  var clientId = options.clientId || process.env.DISTANCE_MATRIX_API_CLIENT_ID;
  var clientSecret = options.clientSecret || process.env.DISTANCE_MATRIX_API_CLIENT_SECRET;
  ...
  function formatRequestUrl(path, query, useClientId) {
    ...
    if (useClientId) {
      ...
      throw 'Missing either a valid API key, or a client ID and secret asd';
    }
